...
function use(reducer, initialState) {
  if (RenderContext$ReView.isActive(/* () */0)) {
    ...
    if (state !== undefined) {
      var match$1 = state;
      if (match$1.tag === /* ReducerState */4) {
        actualState = match$1[0];
      } else {
        throw Exception$ReView.IllegalSlotAccess;
      }
    } else {
      node.setState(current, initialState);
      actualState = initialState;
    }
    var actualDispatch;
    if (dispatch !== undefined) {
      var match$2 = dispatch;
      if (match$2.tag === /* ReducerDispatch */5) {
      ...
      node.setState(current + 1 | 0, /* ReducerDispatch */Block.__(5, [newDispatch]));
